...
describe("Gameboard", () => {
  ...
  describe("ship placement", () => {
    ...
    it("does not place a ship on top or near other ship", () => {
      ...
      expect(
        gameboard.placeShip(new Ship("carrier", 5), shipX + 1, shipY)
      ).toBe(false);

      expect(
        gameboard.placeShip(new Ship("carrier", 5), shipX + 2, shipY)
      ).toBe(true);
    });
  });
  ...
  describe("ship rotation", () => {
    ...
    it("does not rotate ship if invalid", () => {
      expect(gameboard.placeShip(ship, shipX, shipY)).toBe(true);
      expect(
        gameboard.placeShip(new Ship("carrier", 5), shipX + 3, shipY)
      ).toBe(true);
  ...
  describe("ship removing", () => {
    it("removes a ship from board", () => {
      ...
      for (let i = shipY; i < shipY + ship.length; i += 1) {
        expect(gameboard.grid[shipX][i]).toEqual({
          ship: null,
          attacked: false,
        });
      }
